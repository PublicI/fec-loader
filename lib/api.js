...
function queueFilingsToCheck(opts) {
    ...
    models.fec_filing.findAll({
        ...
        .then(filings => {
            ...
            request(`https://api.open.fec.gov/v1/efile/filings/?sort=-receipt_date&per_page=${opts.lookBehind}&api_key=${process.env.FEC_KEY}&page=1&cache=${Math.round(Math.random()*100)}`, function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    ...
                    if (data && data.results) {
                        ...
                        data.results.forEach(filing => {
                            // console.log(filing.file_number,filings.indexOf(filing.file_number));
                            if (!filings.includes(filing.file_number)) {
